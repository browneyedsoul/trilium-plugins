class OutlineByHeadingWidget extends api.NoteContextAwareWidget {
  ...
  doRender() {
    this.$widget = $(
      "<div class='outline-by-heading-widget' style='display:none;'></div>"
    );
  }
  ...
  _getPaneRoot() {
    ...
    return (
      el.closest(".note-detail-pane") || el.closest(".center-pane") || document
    );
  }
  ...
  _onFocusLike(e) {
    ...
    if (
      !target.closest?.(this.READONLY_SELECTOR) &&
      !target.closest?.(this.EDITABLE_SELECTOR)
    )
      return;
  ...
  _onMutations(muts) {
    ...
    const isRelevant = muts.some((m) => {
      const t = m.target;
      const el =
        t?.nodeType === 1 ? t : t?.nodeType === 3 ? t.parentElement : null;
